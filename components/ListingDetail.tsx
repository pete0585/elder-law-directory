...
export default function ListingDetail({ listing, monthlyViews = 0, isClaimed: isClaimedProp }: ListingDetailProps) {
    ...
    return (<div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`bg-white rounded-xl border ${isFeatured ? 'border-brand-amber' : 'border-surface-border'} shadow-sm p-6`}>
            <div className="flex items-start gap-5">
              {listing.photo_url ? (<img src={listing.photo_url} alt={listing.full_name} className="w-20 h-20 rounded-full object-cover border-2 border-brand-slate/20 shrink-0"/>) : (<div className="w-20 h-20 rounded-full bg-brand-slate/10 flex items-center justify-center shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                {listing.firm_name && (<p className="text-gray-600 font-medium mb-2">{listing.firm_name}</p>)}
                ...
                <div className="flex flex-wrap gap-2">
                  {null}
                  {listing.is_cela_certified && (<span className="flex items-center gap-1 text-xs bg-brand-amber/15 text-brand-amber-dark font-semibold px-2.5 py-1 rounded-full">
                      <Award className="w-3.5 h-3.5" aria-label="CELA"/>
                      CELA Certified
                    </span>)}
                  {listing.is_naela_member && (<span className="flex items-center gap-1 text-xs bg-gray-100 text-gray-700 font-semibold px-2.5 py-1 rounded-full">
                      NAELA Member
                    </span>)}
                  {listing.accepts_remote && (<span className="flex items-center gap-1 text-xs bg-blue-50 text-blue-700 font-semibold px-2.5 py-1 rounded-full">
                      <Wifi className="w-3.5 h-3.5" aria-label="Remote"/>
                      Remote Consultations
                    </span>)}
                  {listing.free_consultation && (<span className="flex items-center gap-1 text-xs bg-green-50 text-green-700 font-semibold px-2.5 py-1 rounded-full">
          ...
          {/* Practice Areas */}
          {listing.practice_areas.length > 0 && (<div className="bg-white rounded-xl border border-surface-border shadow-sm p-6">
              <h2 className="font-bold text-gray-900 mb-4">Practice Areas</h2>
              <div className="flex flex-wrap gap-2">
                {listing.practice_areas.map((area) => (<Link key={area} href={`/categories/${area}`} className="text-sm bg-surface border border-surface-border hover:border-brand-slate/40 hover:bg-brand-slate/5 px-3 py-1.5 rounded-full text-gray-700 hover:text-brand-slate transition-colors">
          ...
          {/* States Licensed */}
          {listing.states_licensed.length > 0 && (<div className="bg-white rounded-xl border border-surface-border shadow-sm p-6">
              <h2 className="font-bold text-gray-900 mb-4">States Licensed</h2>
              <div className="flex flex-wrap gap-2">
                {listing.states_licensed.map((s) => (<span key={s} className="text-sm bg-surface border border-surface-border px-3 py-1 rounded-full text-gray-700">
                    {STATE_NAMES[s] ?? s}
                  </span>))}
              </div>
            </div>)}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-brand-slate shadow-sm p-6">
            ...
            <div className="space-y-3">
              {listing.phone && (<a href={`tel:${listing.phone}`} className="flex items-center gap-3 text-sm text-gray-700 hover:text-brand-slate transition-colors group">
              ...
              {listing.email && (<a href={`mailto:${listing.email}`} className="flex items-center gap-3 text-sm text-gray-700 hover:text-brand-slate transition-colors group">
                  <div className="w-8 h-8 bg-brand-slate/10 group-hover:bg-brand-slate/20 rounded-lg flex items-center justify-center transition-colors">
                    <Mail className="w-4 h-4 text-brand-slate" aria-label="Email"/>
                  </div>
                  <span className="truncate">{listing.email}</span>
                </a>)}
              {listing.website && (<a href={listing.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-gray-700 hover:text-brand-slate transition-colors group">
                  <div className="w-8 h-8 bg-brand-slate/10 group-hover:bg-brand-slate/20 rounded-lg flex items-center justify-center transition-colors">
            ...
            {!isVerified && (<div className="mt-4 pt-4 border-t border-surface-border">
                <Link href={`/claim/${listing.id}`} className="block w-full text-center bg-brand-slate hover:bg-brand-slate-dark text-white font-semibold py-2.5 rounded-lg text-sm transition-colors">
                  Claim This Profile
                </Link>
                <p className="text-xs text-gray-500 text-center mt-2"> Listings are free. Public contact details are available without payment. </p>
              </div>)}
          </div>
          ...
          {/* Back to search */}
          <Link href="/listings" className="block text-center text-sm text-brand-slate hover:underline font-medium py-2">
